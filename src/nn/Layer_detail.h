...
void Layer<ActivateMethod, UpdateMethod>::init
	(
	...
	)
{
	...
	weightTDelta  = DeviceMatrix(dim_output, miniBatchSize);
...
template <class ActivateMethod, class UpdateMethod>
void Layer<ActivateMethod, UpdateMethod>::initWeightBias(void)
{
	//weightの初期化
	{
...
//順伝播
template <class ActivateMethod, class UpdateMethod>
const cuda::DeviceMatrix& Layer<ActivateMethod, UpdateMethod>::forward(const DeviceMatrix& x)
{
	//weight, biasを掛ける
...
//u = weight * x + bias * _1B ^ T
template <class ActivateMethod, class UpdateMethod>
void Layer<ActivateMethod, UpdateMethod>::forwardLinear(const DeviceMatrix& x)
{
...
template <class ActivateMethod, class UpdateMethod>
void Layer<ActivateMethod, UpdateMethod>::getWeightTDelta()
{
	//weightTdelta = (weight)^T * delta;
	//<=>
...
//逆伝播
template <class ActivateMethod, class UpdateMethod>
const cuda::DeviceMatrix& Layer<ActivateMethod, UpdateMethod>::back(const DeviceMatrix& weight_t_delta)
{
	//delta = f'(u) ** ((weight)^T * delta_output);
	//"**"は要素ごとの積
	ActivateMethod::getDelta(u, z, weight_t_delta, delta);
...
//パラメータの更新
template <class ActivateMethod, class UpdateMethod>
void Layer<ActivateMethod, UpdateMethod>::update(const DeviceMatrix& x)
{
