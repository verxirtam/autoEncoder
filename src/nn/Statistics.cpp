...
void getMeanAndVarCovMatrix(const DeviceMatrix& sample, DeviceVector& mean, DeviceMatrix& varCovMatrix, cudaStream_t stream)
{
	...
	float alpha = 1.0f;
	float beta = 0.0f;
	cuda::Sgemv(&alpha, CUBLAS_OP_N, X, _1N, &beta, X_1N);
	...
	// X_ = X - mean * _1N^T;
	//		X_ = X;
	//		cuda::Sger(-1.0f, mean, _1N, X_);
	//			<=>
	//			X_ = (-1.0f) * mean * _1N^T + X;
	//			   = X - mean * _1N^T;
	DeviceMatrix X_ = X;
	alpha = - 1.0f;
	cuda::Sger(&alpha, mean, _1N, X_);
	...
	//		cuda::Ssyrk((1 / N), CUBLAS_OP_N, X_, 0.0f, varCovMatrix);
	varCovMatrix = DeviceMatrix(D, D);
	alpha = 1.0f / static_cast<float>(N);
	beta  = 0.0f;
	cuda::Ssyrk(&alpha, CUBLAS_OP_N, X_, &beta, varCovMatrix);
